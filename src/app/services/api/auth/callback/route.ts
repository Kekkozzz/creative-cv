...
export async function GET(request: Request) {
  ...
  let next = requestUrl.searchParams.get("next") ?? "/services/dashboard";
  ...
  const safeNext = next.startsWith("/") ? next : "/services/dashboard";
  ...
  if (!code) {
    const response = NextResponse.redirect(
      `${baseUrl}/services/login?error=auth&reason=missing_code`
    );
  ...
  const response = NextResponse.redirect(
    `${baseUrl}/services/login?error=auth&reason=${encodeURIComponent(error.message)}`
  );
  response.cookies.set("auth_next", "", { path: "/", maxAge: 0 });
  return response;
}
